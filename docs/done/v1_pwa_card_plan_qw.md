...
| `/verify` | Редирект на `/card/{phone}` (уже верифицирован) | Пропустить (верификация) |
...
* **Логика:**
  ...
  - Все параметры установлены в `POST /api/verify`

#### U4.2. Rate Limiting для /api/send-sms (опционально)
...
#### U4.3. HTTPS для продакшена (деплой)
