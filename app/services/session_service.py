...
async def create_session_async(db: AsyncSession, user_id: int, expires_in_days: int = 30) -> str:
    """
    Create new session for user and return token (async version).
...
async def get_session_by_token_async(db: AsyncSession, token: str) -> Optional[Session]:
    """
    Retrieve session by token (async version).
...
async def delete_session_async(db: AsyncSession, token: str) -> bool:
    """
    Delete session (logout user) - async version.
